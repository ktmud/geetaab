...
export const dictionary = {
  ...
  zh: {
    ...
    h1GetTab: '拿一份真弹得下来的谱。',
    ledeParagraph: 'geetaab 会听出和弦、调和速度，再把整首歌改写成你已经会按的那几个指法——变调夹夹几品也一并算好。接着横过屏幕，像唱卡拉 OK 那样跟着练，跟不上就调慢。',
    ...
    printChordChart: '和弦谱',
    printTablature: '六线谱',
    printFoot: 'geetaab 出的机器转谱，请以自己的耳朵为准。',
    ...
    openShapes: (ratio: number) => `这首歌有 ${Math.round(ratio * 100)}% 已经落在开放和弦上，再加变调夹也帮不上什么忙。`,
    ...
    copied: '已复制',
    practiseThis: '开始练这首',
    lowConfidence: '和声听得不太真切，这些和弦只能算大概。录得干净一点，或者直接给它一个音频文件而不是在房间里录，结果会好很多。',
    ...
    learnThemOrder: '按这个顺序学，一次两个——要练的是换和弦，不是按和弦：慢慢数四拍按住一个，再四拍换到下一个，来回倒。开放把位的歌，用到的都跳不出这八个。',
    everyChordBook: '全部和弦',
    anyRoot: '全部根音',
    noBarre: '不用横按',
    barreOnly: '只看横按',
    ...
    practiceHints: [
      (beatsPerBar: number) => `按下播放，先给 ${beatsPerBar} 拍预备拍；之后每个色块走到那条琥珀色线上，就换和弦。`,
    ...
    hwS6Caption: '响度随时间的起伏。每一个峰就是一次拨响，相邻两次之间的距离，就是这首歌的一拍。',
    ...
    hwS89Descs: [
      ...
      '——变化密集的歌最多给三个版本：简单版（收掉七音、挂留音和一带而过的经过和弦）、标准版（默认读法）、忠实版（保留每一个扩展音）。只有当三者确实不同时才会给。',
    ],
    hwS89Final: '成品',
    hwS89Strum: 'v 表示下扫弦',
